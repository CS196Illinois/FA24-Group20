...
def negative_sentences(text_to_summarize, aspect):
    ...
    List them exactly how they are stated in the article. Do not abbreviate with ellipses or things like "etc.". Do not include quotation marks when returning. Please do not include any other information.
    Here is the article:
...
def get_sentence_sentiment_score(phrase, telemetry):
    ...
    if score['compound'] != 0:
        ...
        return tanh(50 * score['compound'])
    if telemetry:
        print(f"{phrase} has a score of {tanh(50 * score['compound'])}")
    return 0

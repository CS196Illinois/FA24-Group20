...
export const CompareContextProvider = ({ children }) => {
	...
	const [file1Label, setFile1Label] = useState(
		`Upload Article 1 (.html) ${String.fromCodePoint(
			8594
		)}`
	);
	const [file2Label, setFile2Label] = useState(
		`Upload Article 2 (.html) ${String.fromCodePoint(
			8594
		)}`
	);
    ...
    const [sentimentPercent1, setSentimentPercent1] = useState(0.0);
    const [sentimentPercent2, setSentimentPercent2] = useState(0.0);
